...
const smsGET = async (req, res) => {
  ...
  if (getSMS) {
    ...
    return res.status(201).json(getSMS);
  }
...
const smsPOST = async (req, res) => {
  const tkn = req.token;
  const dec = await jwtUtility.jwtDecode(req, res, tkn);
  const { clientID } = dec.decoded;
  ...
  setInterval(async () => {
    ...
    if (smsRow !== undefined) {
      for (let i = 0; i < smsRow.length; i++) {
        const { id, receiver, message, statusCode } = smsRow[i];
  ...
  if (triggerSMS.SMSMessageData !== undefined) {
    const scode = triggerSMS.SMSMessageData.Recipients[0].statusCode;
    const { messageId } = triggerSMS.SMSMessageData.Recipients[0];
...
const smswebhookPOST = async (req, res) => {
  console.log('THEY HAVE GOTTEN TO ME IN THE HOOK OOOOOOO ===>> ' + JSON.stringify(req.body));
